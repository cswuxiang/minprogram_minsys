...
class wxValidate {
	...
	__initDefaults() {
		this.defaults = {
			messages: {
				...
				email: '请输入有效的电子邮件地址。',
				tel: '请输入11位的手机号码。',
				smsCode:this.formatTpl('请输入{0}位验证码。'),
				url: '请输入有效的网址。',
				date: '请输入有效的日期。',
				...
				len :  this.formatTpl('请输入{0}位数值。'),
				phone :this.formatTpl('请输入{0}位的手机号码。')
			}
		}
	}
	...
	checkParam(param, rules, event) {
		...
		let value = typeof data == 'string' ? (data||"") : (data[param]||"");
		
		if(value instanceof Array ){//checkbox
			value = value[0]||"";
		}
